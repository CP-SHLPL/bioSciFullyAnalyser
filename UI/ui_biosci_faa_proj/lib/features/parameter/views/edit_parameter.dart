...
import 'package:ui_biosci_faa_proj/features/parameter/data/test_service_provider.dart';
import 'package:ui_biosci_faa_proj/features/parameter/form/parameter_form.dart';
...
class _EditParameterState extends ConsumerState<EditParameter> {
  ...
  Future<void> _fetchDropDowns() async {
    ...
    setState(() {
      blankTypeDropDown.addAll(
        response.dropDowns['BlankType']?.items as List<DropDownListItem>,
      );
      methodDropDown.addAll(
        response.dropDowns['Method']?.items as List<DropDownListItem>,
      );
  ...
  Future<void> _fetchParameterData() async {
    ...
    form
        .control('testID')
        .value = response.testID;
    form
        .control('testCode')
        .value = response.testCode;
    form
        .control('testName')
    ...
    form
        .control('testUnit')
        .value = response.testUnit;
    form
        .control('reagents')
        .value = response.reagents;
    form
        .control('blankType')
        .value = response.blankType;
    form
        .control('testMethod')
        .value = response.testMethod;
    form
        .control('primaryFilter')
        .value = response.primaryFilter;
    form
        .control('secondaryFilter')
        .value = response.secondaryFilter;
    form
        .control('decimals')
        .value = response.decimals;
    form
        .control('incubationTime')
        .value = response.incubationTime;
    form
        .control('readingTime')
        .value = response.readingTime;
    form
        .control('sampleVolume')
        .value = response.sampleVolume;
    form
        .control('r1Volume')
        .value = response.r1Volume;
    form
        .control('r2Volume')
        .value = response.r2Volume;
    form
        .control('autoDilutionRatio')
        .value = response.autoDilutionRatio;
    form
        .control('stirrerSpeedSample')
        .value = response.stirrerSpeedSample;
    form
        .control('stirrerSpeedR2')
        .value = response.stirrerSpeedR2;
    form
        .control('lastReagentBlank')
        .value = response.lastReagentBlank;
    form
        .control('reagentBlankLow')
        .value = response.reagentBlankLow;
    form
        .control('reagentBlankHigh')
        .value = response.reagentBlankHigh;
    form
        .control('corelationA')
        .value = response.corelationA;
    form
        .control('corelationB')
        .value = response.corelationB;
    form
        .control('direction')
        .value = response.direction;
    form
        .control('linearity')
        .value = response.linearity;
    form
        .control('substrateDepletion')
        .value = response.substrateDepletion;
  ...
  void _submitForm() async {
    ...
    final testData = TestData()
      ..mergeFromProto3Json(formValues);
    ...
    testService.updateTest(testResponseData).then((response) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          duration: Duration(
              seconds: 2), // Controls how long it stays on screen
        ),
      );
    });
  }
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: MyReactiveDropDown(
                    ...
                    isEditable: !isEditing,
                    dropDownItems: blankTypeDropDown,
                  ),
                ),
                Spacer(flex: 1),
                Expanded(
                  ...
                  child: MyReactiveDropDown(
                    ...
                    isEditable: !isEditing,
                    dropDownItems: directionDropDown,
                  ),
                ),
                Spacer(flex: 1),
                Expanded(
                ...
                Expanded(
                  ...
                  child: MyReactiveDropDown(
                    ...
                    validationMessages: {
                      ...
                      'min': (error) => 'Primary Filter must be at least 340',
                    },
                  ),
                ),
                Spacer(flex: 1),
              ...
              children: [
                Expanded(
                  ...
                  child: MyReactiveDropDown(
                    ...
                    isEditable: !isEditing,
                    dropDownItems: stirrerSpeedDropDown,
                  ),
                ),
                Spacer(flex: 1),
                Expanded(
